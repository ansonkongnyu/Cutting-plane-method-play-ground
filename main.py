...
def get_policy(policy_params):
    ...
    policy_params['hsize'] = 128
    policy_params['numlayers'] = 3
...
class ARSLearner(object):
    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False,iter=0):
        """ 
        ...
        if self.deltas_used > self.num_deltas:#and iter<randomnum:
    ...
    def train_step(self,iter=0):
        """ 
        Perform one update step of the policy weights.
        """
        
        g_hat = self.aggregate_rollouts(iter=iter)                    
    ...
    def train(self, num_iter):
        ...
        for i in range(num_iter):
            t1 = time.time()
            self.train_step(iter=i/num_iter)
            t2 = time.time()
            ...
            if ((i + 1) % 1 == 0):
                t3 = time.time()
                rewards = self.aggregate_rollouts(num_rollouts = 5, evaluate = True,iter=i/num_iter)
                t4 = time.time()
